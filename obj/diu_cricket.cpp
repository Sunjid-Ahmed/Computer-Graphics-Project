...
float playerX[NUM_PLAYERS] = { 600, 460, 705, 740, 790, 850, 760, 460, 380, 300, 900 };
float playerY[NUM_PLAYERS] = {  34,  20,  34,  50,  68,  90, 118, 118,  88,  55, 118 };
...
void drawStumps(float x, float y)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        glBegin(GL_QUADS);
        glVertex2f(x + i * 6, y);
        glVertex2f(x + i * 6 + 2, y);
        glVertex2f(x + i * 6 + 2, y + 22);
        glVertex2f(x + i * 6, y + 22);
        glEnd();
    }
}
...
// moved) and travels to the striker's stumps
//========================
void drawBall()
{
...
void drawHUD()
{
    ...
    bitmapText(15, HEIGHT - 22, GLUT_BITMAP_HELVETICA_12,
        "P=player  S=student  C=car  1-9,0=jump to entity  TAB=next  Arrows=move");
